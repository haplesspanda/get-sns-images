...
(function newTwitterExecFn() {
  function addLinksToPage() {
    ...
    const structuredItems: StructuredItem[] = actualTweets.map(streamItem => {
      ...
      const imageLinks: HTMLLinkElement[] = Array.from(streamItem.querySelectorAll(photoLinkSelector));
      const filteredImageLinks = imageLinks.filter(imageLink => imageLink.querySelector(innerImageSelector));
      ...
      const images = sortedImageLinks.map(link => link.querySelector(innerImageSelector));
    ...
    structuredItems.forEach(structuredItem => {      
      ...
      if (textAreaContainer && !hasTextArea(textAreaContainer) && structuredItem.imageUrls.length > 0) {
