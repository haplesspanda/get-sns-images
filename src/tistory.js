...
(function tistoryExecFn() {
  ...
  const title = document.querySelector('#content h2') || document.querySelector('.title');
  ...
  const contents = document.querySelector('.contents') || document.querySelector('.article');
  const srcs = Array.from(contents.querySelectorAll('img')).map(img => img.getAttribute('src'));
  
  const textareaParent = document.querySelector('#head') || document.querySelector('.entryTitle') || title;
